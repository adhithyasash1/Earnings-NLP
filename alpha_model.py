# ============================================
# alpha_model.py - Updated with Tuning & Regression
# ============================================
...
class AlphaModel:
    def __init__(self, config: Config):
        ...
        self.model_pipeline: Pipeline = None  # This will hold the *full* pipeline

    def _get_model(self) -> Any:
        """
        Selects the appropriate model based on config.
        """
    ...
    # ... (prepare_labels method remains IDENTICAL) ...
    ...
    def train(self, X: pd.DataFrame, y: pd.Series) -> float:
        """
        Builds the full pipeline and trains, optionally with GridSearchCV.
        """
        preprocessor = self._build_preprocessor(X)
        model = self._get_model()

        # This is our full pipeline: preprocess -> model
        ...
        if self.config.use_hyperparam_tuning:
            ...
            # We can't get a validation score easily without a loop
            # So we just fit the final model
            self.model_pipeline.fit(X, y)
            self.logger.warning("No cross-validation score calculated (tuning is disabled).")
            return -1.0  # Indicate no score
    ...
    def predict(self, X: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if is_classification:
            ...
            # For regression, prediction is the "probability"
